//! Explicit, bounded V1.5 reconciliation plus MemPalace delivery.
//!
...
const DEFAULT_CONSUMER_ID: &str = "mempalace";

fn main() {
    if let Err(error) = run(env::args().skip(1).collect()) {
        eprintln!("agent-bookkeeper-mempalace-controller: {error}");
        process::exit(2);
    }
}
...
fn run(arguments: Vec<String>) -> Result<(), String> {
    ...
    .map_err(display_error)?;
    let runner = MempalaceCommandRunner::new(
        MempalaceCliConfig::new(config.mempalace_executable, config.palace_path, config.wing)
            .map_err(display_error)?,
    );
    let mut consumer =
        MempalaceConsumer::new(config.receipt_root, runner).map_err(display_error)?;
...
impl ControllerConfig {
    fn parse(arguments: Vec<String>) -> Result<Self, String> {
        ...
        let config = Self {
            ...
            mempalace_executable: absolute_path(
                required(&mut values, "--mempalace-executable")?,
                "--mempalace-executable",
            )?,
            palace_path: absolute_path(required(&mut values, "--palace")?, "--palace")?,
            wing: required(&mut values, "--wing")?,
        ...
        if config.wing.trim() != config.wing || config.wing.is_empty() {
            return Err("--wing must be non-empty and trimmed".to_owned());
        }
